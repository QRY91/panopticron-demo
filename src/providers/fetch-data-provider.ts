...
const fetchDataProvider: DataProvider = {
  ...
  custom: async ({ url, method, payload, query, headers: initialHeaders }) => {
    ...
    if (DEMO_CONFIG.USE_MOCK_DATA) {
      return { data: { success: true, message: "Mock data response" } };
    }
    ...
    requestHeaders.set("apikey", "mock-anon-key");
    ...
    try {
      ...
      if (
        ...
        if (!response.ok && response.status !== 204) {
          // Still an error if not ok and not 204
  ...
  getList: async () => {
    throw new Error("FetchDataProvider: getList not implemented");
  },
  getOne: async () => {
    throw new Error("FetchDataProvider: getOne not implemented");
  },
  create: async () => {
    throw new Error("FetchDataProvider: create not implemented");
  },
  update: async () => {
    throw new Error("FetchDataProvider: update not implemented");
  },
  deleteOne: async () => {
    throw new Error("FetchDataProvider: deleteOne not implemented");
  },
};
