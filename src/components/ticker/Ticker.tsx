...
import { Box, Typography, Link as MuiLink, useTheme, alpha } from '@mui/material';
...
function getStatusIconElement(status?: string): React.ReactElement {
    const iconProps = { fontSize: "inherit" as "inherit", sx: { mr: 0.5, verticalAlign: 'middle' } };
...
const Ticker: React.FC<TickerProps> = ({ initialItems = [], speed = 80, refreshInterval = 60000 }) => {
  ...
  useEffect(() => {
    const fetchWorkerStatusesForTicker = async () => {
        ...
        for (const name of workerNames) {
            ...
            const friendlyName = name.replace('sync-', '').replace('-projects','').replace('-data','').split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
            ...
            if (error && error.code !== 'PGRST116') { // PGRST116 = 0 rows
                ...
                newTickerItems.push({
                    id: `worker-${name}-${data.ended_at ? new Date(data.ended_at).getTime() : Date.now()}`,
                    message: `${friendlyName} Sync: ${data.status}`,
                    type: data.status === 'Success' || data.status === 'No Action Needed' ? 'success' : (data.status === 'Failure' ? 'error' : (data.status === 'Partial Success' ? 'warning' : 'system')),
    ...
    const intervalId = setInterval(fetchWorkerStatusesForTicker, refreshInterval);

    return () => clearInterval(intervalId);
  }, [refreshInterval]);


  const allItems = useMemo(() => [...systemWorkerItems, ...initialItems], [systemWorkerItems, initialItems]);
  
  // Duplicate items for a smoother visual loop
  const displayItems = allItems.length > 0 && allItems.length < 10 ? [...allItems, ...allItems, ...allItems] : allItems;
  
  const typeStyles = {
    info: { color: theme.palette.info.main, defaultIcon: <InfoIcon fontSize="inherit" sx={{mr: 0.5, verticalAlign: 'middle'}} /> },
    warning: { color: theme.palette.warning.main, defaultIcon: <WarningAmberOutlinedIcon fontSize="inherit" sx={{mr: 0.5, verticalAlign: 'middle'}} /> },
    error: { color: theme.palette.error.main, defaultIcon: <ErrorOutlineIcon fontSize="inherit" sx={{mr: 0.5, verticalAlign: 'middle'}} /> },
    success: { color: theme.palette.success.main, defaultIcon: <CheckCircleOutlineIcon fontSize="inherit" sx={{mr: 0.5, verticalAlign: 'middle'}} /> },
    system: { color: theme.palette.text.secondary, defaultIcon: <HelpOutlineIcon fontSize="inherit" sx={{mr: 0.5, verticalAlign: 'middle'}} /> }, // Default for system
  };

  if (allItems.length === 0) {
    return (
      <Box sx={{ flexGrow: 1, textAlign: 'center', py: 1.5, opacity: 0.7, overflow: 'hidden' }}>
  ...
  return (
    <Box
      ...
      sx={{
        flexGrow: 1, overflow: 'hidden', whiteSpace: 'nowrap',
        cursor: 'default', py: 0.5, mx: 2, position: 'relative',
      }}
      ...
      <Box
        ...
        sx={{
          ...
          '@keyframes scroll-duplicated': {
            ...
            '100%': { transform: `translateX(-${100 / (displayItems.length / allItems.length || 1)}%)` }, // Avoid division by zero
          },
        }}
      >
        {displayItems.map((item, index) => {
          ...
          return (
            <Box
              ...
              key={`${item.id}-${index}`}
              sx={{
                display: 'inline-flex', alignItems: 'center', mx: 2.5, py: 0.5,
                color: itemStyle.color,
                transition: 'opacity 0.2s, transform 0.2s',
                opacity: isHovering ? 0.8 : 1,
                transform: 'scale(1)',
                '&:hover': { opacity: 1, transform: 'scale(1.03)', fontWeight: '600' }
              }}
            >
              {React.cloneElement(iconToDisplay, { sx: { verticalAlign: 'middle', mr: 0.5, color: itemStyle.color }})}
              <Typography variant="body2" component="span" sx={{lineHeight: 1}}>
                ...
                {item.timestamp && (
                  <Typography variant="caption" component="span" sx={{ ml: 0.5, opacity: 0.7, fontStyle: 'italic' }}>
                    ({item.timestamp})
                  </Typography>
                )}
              </Typography>
